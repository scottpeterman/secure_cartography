#!/usr/bin/env python3
import json
import sqlite3
import logging
from pathlib import Path
from typing import Dict, Optional


class BaseLoader:
    def __init__(self, db_path: str, verbose: bool = False):
        self.db_path = db_path
        self.verbose = verbose
        self.logger = self._setup_logger()
        self.conn = self._setup_database()

    def _setup_logger(self):
        log_level = logging.DEBUG if self.verbose else logging.INFO
        logging.basicConfig(
            level=log_level,
            format='%(asctime)s - %(name)s - %(levelname)s - %(message)s'
        )
        return logging.getLogger(self.__class__.__name__.lower())

    def _setup_database(self) -> sqlite3.Connection:
        self.logger.info(f"Connecting to database: {self.db_path}")
        conn = sqlite3.connect(self.db_path)
        conn.execute("PRAGMA foreign_keys = ON")
        return conn

    def _get_device_id(self, device_name: str) -> Optional[int]:
        cursor = self.conn.cursor()
        sql = "SELECT device_id FROM devices WHERE name = ?"
        self.logger.debug(f"Looking up device ID for: {device_name}")
        cursor.execute(sql, (device_name,))
        result = cursor.fetchone()
        return result[0] if result else None

    def close(self):
        if self.conn:
            self.conn.close()


class InterfaceLoader(BaseLoader):
    def _store_interface(self, device_id: int, name: str, interface_data: Dict):
        cursor = self.conn.cursor()
        status = interface_data.get('status', {})
        hardware = interface_data.get('hardware', {})
        counters = interface_data.get('counters', {})

        sql = """
            INSERT OR REPLACE INTO device_interfaces (
                device_id, name, link_status, protocol_status,
                interface_type, mac_address, bia, mtu, bandwidth,
                duplex, speed, media_type, description, ip_address,
                input_packets, output_packets, input_errors, output_errors,
                last_updated
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
        """

        params = (
            device_id, name, status.get('link'), status.get('protocol'),
            hardware.get('type'), hardware.get('mac_address'), hardware.get('bia'),
            hardware.get('mtu'), hardware.get('bandwidth'), hardware.get('duplex'),
            hardware.get('speed'), hardware.get('media_type'),
            interface_data.get('description'), interface_data.get('ip'),
            counters.get('input_packets'), counters.get('output_packets'),
            counters.get('input_errors'), counters.get('output_errors')
        )

        cursor.execute(sql, params)

    def process_interface_file(self, filepath: Path) -> str:
        self.logger.info(f"Processing interface file: {filepath}")
        try:
            with open(filepath) as f:
                data = json.load(f)

            success_count = 0
            error_count = 0

            for device_name, device_data in data.get('successful', {}).items():
                try:
                    device_id = self._get_device_id(device_name)
                    if not device_id:
                        self.logger.error(f"Device {device_name} not found in database")
                        error_count += 1
                        continue

                    self.conn.execute("BEGIN TRANSACTION")
                    interfaces = device_data.get('interfaces', {}).get('interfaces', {})
                    self.logger.debug(f"Processing {len(interfaces)} interfaces for device {device_name}")

                    for interface_name, interface_data in interfaces.items():
                        self._store_interface(device_id, interface_name, interface_data)

                    self.conn.commit()
                    success_count += 1
                    self.logger.info(f"Successfully processed interfaces for {device_name}")

                except Exception as e:
                    self.conn.rollback()
                    self.logger.error(f"Error processing device {device_name}: {str(e)}")
                    error_count += 1

            return f"Processed {filepath.name}: {success_count} devices successful, {error_count} failed"

        except Exception as e:
            self.logger.error(f"Error processing file {filepath}: {str(e)}")
            return f"Failed to process {filepath.name}: {str(e)}"


def main():
    import argparse
    parser = argparse.ArgumentParser(description='Load interface data into CMDB')
    parser.add_argument('--db', required=True, help='Path to the SQLite database')
    parser.add_argument('--base-path', required=True, help='Base path containing site folders')
    parser.add_argument('--site', help='Process specific site (optional)')
    parser.add_argument('-v', '--verbose', action='store_true', help='Enable verbose output')

    args = parser.parse_args()
    base_path = Path(args.base_path)
    loader = InterfaceLoader(args.db, args.verbose)
    print(f"upload_interfaces.py args: {args}")
    try:
        if args.site:
            # Process single site
            site_dir = base_path / args.site
            if not site_dir.is_dir():
                print(f"Site directory not found: {site_dir}")
                return
            interface_file = site_dir / f"device_interfaces.json"
            if interface_file.exists():
                result = loader.process_interface_file(interface_file)
                print(result)
            else:
                print(f"Interface file not found: {interface_file}")
        else:
            # Check for interface file directly in base path
            base_path = Path(base_path).resolve()
            interface_file = base_path / "home_interfaces.json"
            print(f"Looking for interface file: {interface_file}")
            if interface_file.exists():
                print(f"Found interface file: {interface_file}")
                result = loader.process_interface_file(interface_file)
                print(result)
            else:
                print(f"Interface file not found: {interface_file}")
    finally:
        loader.close()


if __name__ == '__main__':
    main()