...
from graphml_layoutmgr import LayoutManager
...
class NetworkGraphMLExporter:
    ...
    def load_icons(self) -> None:
        """Load and configure icons from JSON configuration and convert JPG files directly"""
        try:
            # Load the platform icon map configuration
            ...
            for icon_file in unique_icons:
                try:
                    ...
                    base_name = Path(icon_file).stem  # Get filename without extension
                    icon_base_path = Path(config.base_path) / base_name
                    jpg_path = icon_base_path.with_suffix('.jpg')
                    jpeg_path = icon_base_path.with_suffix('.jpeg')
                    ...
                    # Convert JPG directly to base64
                    with open(icon_path, 'rb') as f:
                        # Convert image file to base64
                        base64_data = base64.b64encode(f.read()).decode('utf-8')

                        # Clean the base64 data
                        icon_data = base64_data.strip()

                        # Store using the original filename as key
                        self.icons[icon_file] = icon_data
    ...
    def is_endpoint(self, node_id: str, node_data: dict) -> bool:
        ...
        endpoint_keywords = {'endpoint', 'camera', 'wap', 'ap', 'phone', 'printer'}
    ...
    def preprocess_topology(self, network_data: dict) -> dict:
        """Add missing node definitions for all referenced nodes"""
        # Create sets of defined and referenced nodes
        defined_nodes = set(network_data.keys())
        referenced_nodes = set()

        # Find all referenced nodes - only from peers, not from connections
        for node_data in network_data.values():
            if 'peers' in node_data:
                for peer_id, peer_data in node_data['peers'].items():
                    referenced_nodes.add(peer_id)

        # Create node definitions for undefined nodes
        ...
        # Filter out endpoints if not including them
        if not self.include_endpoints:
            filtered_topology = {}
            for node_id, node_data in enhanced_topology.items():
                if not self.is_endpoint(node_id, node_data):
                    # Filter peers to remove endpoint connections
                    if 'peers' in node_data:
                        filtered_peers = {}
                        for peer_id, peer_data in node_data['peers'].items():
                            if not self.is_endpoint(peer_id, enhanced_topology.get(peer_id, {})):
                                filtered_peers[peer_id] = peer_data
                        node_data = node_data.copy()
                        node_data['peers'] = filtered_peers
                    filtered_topology[node_id] = node_data
